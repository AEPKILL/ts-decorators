...
export function AfterAsync(
  afterAsyncRun: (self: any, result: any, ...args: any[]) => void | AopResult
) {
      ...
      function aop_before_run(
        ...
        if (isPromise(promise)) {
          return promise.then(result => {
            const aopResult: AopResult = afterAsyncRun.apply(
              self,
              [self, result].concat(args)
            );
          ...
          throw new TypeError(
            `AfterAsync装饰器必须装饰一个异步构造函数或方法 , ${targetNameString} 并不是一个异步的构造函数或方法`
          );
        }
      }
    ) as T;
