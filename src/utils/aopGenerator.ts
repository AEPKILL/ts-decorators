...
import decorateTarget, { DECORATE_TARGET } from './decorateTarget';

// tslint:disable:ban-types

export function aopGenerator(
  target: Function,
  descriptor: PropertyDescriptor,
  aop: (targetFn: Function, self: any, args: any[]) => any
  ...
  const targetFunction: Function =
    targetType === DECORATE_TARGET.CLASS ? target : descriptor.value;
  const wapper = staticExtend(
    createFunctionWithName(targetFunction.name, function F(
      this: any,
      ...args: any[]
    ) {
